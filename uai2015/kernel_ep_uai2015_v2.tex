...
\newcommand{\witta}[1]{\color{blue!50!black}{#1} }
...
any kind of factor that can be sampled.\todo{AE: 1. State the problem. 2. Say why it's interesting. 3. Say what your solution achieves. 4. Say what follows from your solution. Message-passing is useful. Message-passing in general is slow. Our approach speeds it up. This allows us to use it on more complicated / larger problems.}
\end{abstract}

\section{Introduction}

\begin{itemize}
\item In general: learning to infer is becoming a THING (variational community, inference networks for NN; probabilistic programming literature [kernel EP, stochastic inverses, Wingate stuff, etc.; can have additional properties beyond being accurate like better convergence properties (Eslami); all super-awesome and we need more research).
\item Inference is hard (several problems: global [need to manipulate many interdependent variables]; unclear how to represent distributions [which, in the continuous case, can be infinite-dimensional objects], update rules are typically intractable [need to solve non-trivial integrals].
\item Message passing reduces inference to local, deterministic update rules.
\item VMP, EP solve the question of representation of distribution by mapping onto simple parametric forms, but the problem remains that we need to map / transform messages.
\item remaining fundamental question: how to manipulate distributions or messages (distribution-to-distribution mappings).
\item Barthelme, Heess, Eslami propose solutions, the first is asymptotically exact and principled but stochastic and therefore slow / high variance (this is especially bad as it is an inner loop to MP algorithm); Heess learns a deterministic mapping in a batch setup with NNs but does not produces uncertainty estimates and therefore blows up without warning, no factor refinement, need to do a lot of (potentially unnecessary) hard work up front; Eslami combines the two approaches by learning a deterministic mapping on the fly but uses several crude heuristics, computationally relatively expensive online updates and ultimately unprincipled uncertainty estimates.
\item Here we present a kernel-based method for just-in-time learning of distribution-to-distribution mappings and apply it to learning message passing computations (KJIT). KJIT is fast, statistically efficient and its uncertainty estimates are principled.
\item Our first contribution is to develop an N-independent kernel based method for regressing from distributions to distributions.
\item Our second contribution is to apply this to the JIT setting.
\item Experiments on a number of models demonstrate state-of-the-art speed and accuracy.
\end{itemize}
...
Our paper is structured as follows: In section \ref{sec:EP} we briefly review the notion of message passing for approximate inference and, in particular, the expectation propagation (EP) algorithm which is the basis of our work. We then describe how approximate EP can be performed using Monte Carlo estimates of the message, in the case when message updates cannot be computed analytically. In section \ref{sec:Online} we describe how this approximation technique can be improved with an online learning framework that effectively remembers previously computed approximate updates and we propose a novel kernel-based regression technique for this approach.
...
\end{equation}
Here $\factor_j$, the "factors", are non-negative functions which are defined over subsets of the variables $\fx{\factor_j}$, i.e.\ the neighbors of the factor node $\factor_j$ in the factor graph (we use $\fis{\factor_j}$ to denote the set indices of the variables of factor $\factor_j$). $Z$ is the normalization constant. In this work we focus on \textit{directed} factors $\factor(\xout | \xin)$ which specify a conditional distribution over variables $\xout$ given $\xin$ (and $\fx{\factor} = (\xout, \xin))$.
...
EP introduces an approximation in the case when the message $\msg{ \factor }{i}(\cdot)$ does not have a simple parametric form, by projecting the exact marginal $\msg{ i }{\factor}(x_i)\msg{ \factor }{i}(x_i)$ onto a member of some class of known parametric distributions
\begin{equation}
\msg{ \factor }{i}(x_i) = \frac{\projP{ \msg{i}{\factor}(x_{i})
\int \factor (\fx{\factor}) \prod_{i' \in \fis{\factor} \textbackslash i} \msg{i'}{\factor}(x_{i'}) \diffd \bx_{\fis{\factor} \textbackslash i}}}
{\msg{i}{\factor}(x_{i})}.
...
Expectation propagation \citep{Minka2001,Bishop2006} is a message passing algorithm that extends BP in situations where As is common for message passing algorithms we assume that the probabilistic models of interest are represent as factor graphs
...
is known as a cavity distribution. \todo{AE: Too many inline equations.} 
...
techniques are often applied to approximate the integral. \todo{AE: This is the main point we want to get accross. Either needs more emphasis or should be mentioned sooner.}
...
of incoming messages. \todo{AE: This sentence needs reworking now that you only train on messages observed in the model.}
...
\todo{AE: Too many inline equations in this section. Pull out the most important ones?} We consider here the problem of regressing smoothly from distribution-valued
...
of mean parameters.\todo{AE: Best not to use `in our setting' until section 4 (proposed method). This section is still `background', right?} Although this property is appealing, the training
...
\todo{AE: Best not to use `we propose' until section 4 (proposed method).}To eliminate the dependency on $N$, we propose to apply random Fourier
...
\hl{TODO}
\begin{itemize}
\item Add forest cost computed by Ali. Compare it with online kernel ridge
regression.
\end{itemize}
...
operator will query the \todo{AE: Need to talk about online training, concept of an oracle, concept of querying oracle when uncertain.}oracle when there are incoming messages from
...
\end{itemize}
\item Do we need kNN as a baseline regression function? \todo{AE: Maybe enough to point to JIT paper and say KNN performs badly there?}
...
in the message space in a useful way. \todo{AE: This would be nice but is probably not critical. Could be one of the last experiments you run?}
\end{itemize}
...
and $m_{Z\rightarrow f}(z)=\text{Beta}(z;\alpha,\beta)$. \todo{AE: I think at this point you need to properly define the logistic regression model otherwise incoming messages will be incomprehensible.} We randomly
...
to 2000. \todo{AE: Why 2000?} We report $\log KL[q\|\hat{q}]$ where $q=q_{f\rightarrow X}$
...
instead of the expectations of the first two moments. \todo{AE: No need to mention log-scaling?} The histogram
...
an appropriate distribution was one of the unsolved problems in \cite{Heess2013}. \todo{AE: These paragraphs are very informative. Would be great to have them in some form towards the beginning of the paper too. 1. Say what you're going to say. 2. Say it. 3. Say what you just said. there }
...
follows that the posterior of $w$ is given by 
\begin{align*}
p(w\mid Y) & =\mathcal{N}(w;\mu_{w},\Sigma_{w})\\
\Sigma_{w}^{-1} & =XX^{\top}\sigma_{y}^{-2}+\sigma_{0}^{-2}I\\
\mu_{w} & =\Sigma_{w}XY^{\top}\sigma_{y}^{-2}=\left(XX^{\top}+\frac{\sigma_{y}^{2}}{\sigma_{0}^{2}}I\right)^{-1}XY^{\top}.
\end{align*}
The quantity $\sigma_{y}^{2}$ is proportional to the regularization
parameter in linear regression. The predictive distribution on the
output $y^{*}$ given an observation $x^{*}$ is
\begin{align*}
...
\end{align*}
where $K=X^{\top}X$ is a gram matrix of $X$. \textbf{TODO:} We should
be able to write the marginal likelihood in terms of covariance matrix
instead of a gram matrix. One can do gradient ascent on $\log p(Y|X)$
to optimize $\sigma_{0}^{2},\sigma_{y}^{2}$ and kernel parameters.
In our application, we can treat each output (expected sufficient
statistic) separately. Treating all outputs jointly can be achieved
with a multi-output kernel (see \citesup{Alvarez2011}).
...
\begin{align*}
\Sigma_{w}^{(N+1)} & =\left(XX^{\top}\sigma_{y}^{-2}+x_{N+1}x_{N+1}^{\top}\sigma_{y}^{-2}+\sigma_{0}^{-2}I\right)^{-1}\\
...
\end{align*}
where we used Sherman-Morrison formula
\[
\left(A+uv^{\top}\right)^{-1}=A^{-1}-\frac{A^{-1}uv^{\top}A^{-1}}{1+v^{\top}A^{-1}u}
\]
with $A=\Sigma_{w}^{-1(N)}$. Updating $\Sigma_{w}$ costs $O(D^{2})$
per one new observation. 

For $\mu_{w}=\Sigma_{w}XY^{\top}\sigma_{y}^{-2}$, we should maintain
$XY^{\top}\in\mathbb{R}^{D}$ and update it with
\[
\left(XY^{\top}\right)^{(N+1)}=\left(XY^{\top}+x_{N+1}y_{N+1}\right).
\]
One update of $\mu_{w}$ costs $O(D)$. In the first iteration when
no data are observed, $\Sigma_{w}^{(1)}=\sigma_{0}^{2}I$.


\subsection{Active Learning}

If the predictive variance on the current incoming messages $x^{*}$
is high, then we query the outgoing message from the importance sampler
(oracle) and update the $\Sigma_{w}$ and $\mu_{w}$. Otherwise, the
outgoing message is efficiently computed by the operator. We have
$D_{y}$ regression functions. Thus, for each $x^{*}$, we have $D_{y}$
predictive variances, $v_{1}^{*},\ldots,v_{D_{y}}^{*}$, one for each
output. Given a new input $x^{*}$, if $v_{1}^{*}>\tau_{1}$ or $\cdots$
or $v_{D_{y}}^{*}>\tau_{D_{y}}$ where $\{\tau_{i}\}_{i}$ are pre-specified
predictive variance thresholds, then a query is made to the oracle
to obtain a ground truth $y^{*}$. The pair $(x^{*},y^{*})$ is then
used to update the operator as described previously. Querying the
oracle incurs a high computational cost.
...
\subsection{Gaussian Kernel on Mean Embeddings }

Given two distributions $p$ and $q$, a Gaussian kernel on mean embeddings
\cite{Christmann2010} is defined as 
\[
\kappa_{\text{gauss}}(p,q)=\exp\left(-\frac{\|\mu_{q}-\mu_{q}\|_{\mathcal{H}}^{2}}{2\gamma^{2}}\right).
\]
This kernel has two parameters, one for $k$ in $\mu_{q}=\mathbb{E}_{p}k(x,\cdot)$
and $\gamma^{2}$. Empirically, this kernel performs significantly
better than other kernels on the sigmoid factor problem as in \cite{Heess2013}.
We can also parametrise the kernel with an exponent $q$ as
\[
\kappa(p,q)=\exp\left(-\frac{\|\mu_{q}-\mu_{q}\|_{\mathcal{H}}^{q}}{2\gamma^{2}}\right)
\]
where $q\in(0,2]$. We might be able to treat $q$ as a hyperparameter
and tune it by a gradient ascent on the marginal likelihood.


\subsubsection{Random features for Gaussian kernel on mean embeddings}

One straightforward way to get random features for the kernel is given
by a two-stage approximation procedure by Zoltan and Dino.
\[
\kappa_{\text{gauss}}(p,q)=\exp\left(-\frac{1}{2\gamma^{2}}\left\langle \mu_{p},\mu_{p}\right\rangle +\frac{1}{\gamma^{2}}\left\langle \mu_{p},\mu_{q}\right\rangle -\frac{1}{2\gamma^{2}}\left\langle \mu_{q},\mu_{q}\right\rangle \right).
\]
...
we have
\[
\kappa_{\text{gauss}}(p,q)\approx\exp\left(-\frac{\|\hat{\phi}(p)-\hat{\phi}(q)\|_{D_{in}}^{2}}{2\gamma^{2}}\right):=k_{\text{gauss}}\left(\hat{\phi}(p)-\hat{\phi}(q);\gamma^{2}\right)
\]
...
\]
where $\hat{\psi}(p)\in\mathbb{R}^{D_{out}}$. 
...
of $p$ and $q$.
\begin{enumerate}
\item Compute the Fourier transform $\hat{k}$ of the kernel $k$.
\item Draw $D_{in}$ i.i.d. samples $\omega_{1},\ldots,\omega_{D_{in}}\in\mathbb{R}^{d}$
from $\hat{k}$.
\item Draw $D_{in}$ i.i.d samples $b_{1},\ldots,b_{D_{in}}\in\mathbb{R}$
from $U[0,2\pi]$
\item $\hat{\phi}(p)=\mathbb{E}_{p(x)}\sqrt{\frac{2}{D_{in}}}\left(\cos\left(\omega_{1}^{\top}x+b_{1}\right),\ldots,\cos\left(\omega_{D_{in}}^{\top}x+b_{D_{in}}\right)\right)^{\top}$.
If $p(x)=\mathcal{N}(x;m_{p},V_{p})$, $\hat{\phi}(p)=\sqrt{\frac{2}{D_{in}}}\left(\cos(w_{1}^{\top}m_{p}+b_{1})\exp\left(-\frac{1}{2}w_{1}^{\top}V_{p}w_{1}\right),\ldots\right)^{\top}$.
Even if $p$ is not a normal distribution, we may still use it as
an approximation.
\item Draw $D_{out}$ i.i.d. samples $\nu_{1},\ldots,\nu_{D_{out}}\in\mathbb{R}^{D_{in}}$
from $\hat{k}_{\text{gauss}}(\gamma^{2})$ i.e., Fourier transform
of $k_{\text{gauss}}(\cdot,\cdot\gamma^{2})$.
\item Draw $D_{out}$ i.i.d. samples $c_{1},\ldots,c_{D_{out}}\in\mathbb{R}$
from $U[0,2\pi]$.
\item $\hat{\psi}(p)=\sqrt{\frac{2}{D_{out}}}\left(\cos\left(\nu_{1}^{\top}\hat{\phi}(p)+c_{1}\right),\ldots,\cos\left(\nu_{D_{in}}^{\top}\hat{\phi}(p)+c_{D_{out}}\right)\right)^{\top}$.
\end{enumerate}
...
\paragraph{Experiments on Random Features }
...
\section{Experiments on Kernel-Based Message Operators}


\subsection{Actual EP messages. Mix of multiple EP runs. Predictive variance.}

As before, we consider the same dataset i.e. messages collected from
actual EP. Train/test = 5000/2000. In this experiment, we run Steinwart
(Gaussian kernel on mean embeddings) kernel on joint embeddings with
random Fourier features (two staged approximation). 1000 outer features,
500 inner features. We compute predictive variance as in Bayesian
linear regression. The regularization parameter is now proportional
to the output noise in Bayesian linear regression. 


\paragraph{Backward direction (output: Normal message)}

DNormalLogVarBuilder means regressing to Gaussian mean and $\log$
variance.

\includegraphics[width=7cm]{img/pred_var/fm_kgg_joint_-bw-out1-crop}
\includegraphics[width=7cm]{img/pred_var/fm_kgg_joint_-bw-out2-crop}


\paragraph{Forward direction (output: Beta message)}

DBetaLogBuilder means regressing to $\log\alpha$ and $\log\beta$.

\includegraphics[width=7cm]{img/pred_var/fm_kgg_joint_-fw-out1-crop}
