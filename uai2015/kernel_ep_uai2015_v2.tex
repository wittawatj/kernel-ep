...
Message approximations as in the previous section could be used directly when running the EP algorithm as in \cite{Barthelme2011}, but this approach can suffer from the unreliability of the importance sampling estimates when the number of samples $N$ is small. On the other hand, for large $N$ the computational cost of running EP with approximate messages can be very high, as the message approximations have to be re-computed in each iteration of EP. To obtain more reliable message approximations at lower computational cost \cite{Heess2013,Eslami2014} both amortize previously computed approximate message by training a function approximator to directly map a tuple of incoming variable-to-factor messages $(\msg{i'}{\factor} )_{i' \in \fis{\factor}}$ to an approximate factor to variable message $\msg{\factor}{i}$. (To understand this note that any message in EP can be represented by a low-dimensional parameter vector, typically corresponding to the parameters of an exponential family distribution, and the message update (\ref{eq:msgPassing:EP}) can thus be understood as a vector valued function.) \cite{Heess2013} use a fixed training set to train their approximate message operator prior to running EP with the intractable factor; \cite{Eslami2014} investigate an online learning approach in which the approximate message operator can is updated on the fly as required:
