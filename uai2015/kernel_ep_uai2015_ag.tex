...
conjugate prior for standard deviation, has quite pathological behaviour  \citep{Gelman2006}.
In general, more expressive, freely chosen models are more likely
to require expensive sampling or quadrature approaches in their implementation, which can make
them challenging to implement or impractical to run.

We address the particular setting of expectation propagaion \citep{Minka2001}, a message
...
While this is exact for sufficient samples, this sampling procedure must be run at every iteration during inference,
...
the model online in the event a prediction is uncertain). This can be a problem
...
costly, with training time of $O(n^3)$ and a cost of $O(n^2)$ to make a prediction. 
One approach proposed to  address this problem is a random feature approach \citep{Rahimi2007,Le2013}, where
random features of the inputs are computed, however this approach does not apply when
the inputs are distributions. We introduce a novel two-level random feature approach for kernels
on distributions, which gives us both fast prediction (linear in the number of
...
that of \citet{Eslami2014}, with brief details on the random forest regression, the uncertainty computation
procedure, and the overall  computational cost.
...
For the purposes of fast and efficient representation, we provide a random feature-based implementation
of the regression, which may be of independent interest.%: this has two layers, since features of the distribution
%embeddings are fed into features of the kernel on the embeddings.
Finally, in Section \ref{sec:Experiments}, we provide experiments, which cover three topics. First, we show that our
...
\begin{figure}[t]
	...
	\caption{Classification performance and inference times in the logistic 
	regression problem.}
	\label{fig:logistic_performance}
\end{figure}
...
\begin{figure}[t]
	\centering
	\subfloat[KL divergence]{
	%\includegraphics[width=0.49\columnwidth]{img/online/logistic_01_loss-crop}
	\missingfigure[figwidth=0.49\columnwidth]{}
	}
	%
	\subfloat[Inference time\label{fig:cg_inference_time}]{
	\includegraphics[width=0.49\columnwidth]{img/online/cg_inference_time-crop}
	}
	\caption{KL divergence of the posteriors and inference times in the compound 
	Gamma problem.}
	\label{fig:logistic_performance}
\end{figure}
...
text if there is enough space. If not, put it in the appendix. This 
provides an explanation for why we chose the Gaussian kernel on joint 
mean embeddgins.
