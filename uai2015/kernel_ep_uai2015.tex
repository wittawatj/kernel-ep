...
\includegraphics[width=0.9\columnwidth]{img/binary_logistic_regression-crop}
% \begin{tikzpicture}
%  \node[obs] (x) {$x_i$};
...
We present the operator with 50 problems where a new 
set of observations $\{(x_i, y_i)\}_{i=1}^n$ are generated in each problem 
from the model while keeping $\boldsymbol{w}$ fixed.
...
of random features are $D_{in} = 300$ and $D_{out} = 500$. The noise variance 
$\sigma^2_y$ is fixed to $10^{-4}$ and the uncertainty threshold on the log 
predictive variance is fixed to -8.5. To simulate a black
box setup, we use an importance sampler as the oracle where the proposal 
distribution is fixed to $\mathcal{z; 0, 200}$ with 50000 particles.

\figref{fig:logistic_temporal_uncertainty} shows a trace of predictive variance 
of KJIT in predicting the mean of each $\msg{\psi}{z_i}$ on each factor invocation. 
The black dotted lines
mark the start of a new inference problem. The first 300 factor invocations 
correspond to the initial minibatch training; hence
It can be seen that the uncertainty
...
\caption{Uncertainty estimate of KJIT for all incoming messages at each time point 
in the compound Gamma problem. \hl{WJ: Due to the space constraint, we may have to 
remove this figure.}
