...
of the distributions to a reproducing kernel Hilbert space), and the outputs are vectors of message
...
This is possible because of Bochner's theorem which states that a
continuous, translation-invariant kernel can be written in the form of an
inverse Fourier transform, where the Fourier transform of the kernel can be
...
visible even at the 99th percentile. See \secref{sec:batch_learning_detail} in
...
cross validation for parameter selection; however for computational simplicity
we set the kernel parameters (i.e., $\gamma$ in the kernel $\kappa$) according
to the median heuristic
\citep{Scholkopf2002}. 
...
& \textbf{mean log KL} & \textbf{s.d. of log KL}  \\\hline
 ...
 Breiman's random forests \citep{Breiman2001} & -8.69 & 1.79\\
 Extremely randomized trees \citep{Geurts2006} & -8.90 & 1.59 \\
 \citeauthor{Eslami2014}'s random forests \citep{Eslami2014} & -6.94 & 3.88 \\
 \hline
\end{tabular}
\end{center}
...
to  Breiman's random forests \citep{Breiman2001}, extremely randomized trees
\citep{Geurts2006}, and \citeauthor{Eslami2014}'s random forests. We use
scikit-learn 
toolbox for the extremely randomized trees and Breiman's random forests. For
\citeauthor{Eslami2014}'s random forests, we reimplemented the method as
closely as possible according to the description given in \citet{Eslami2014}.  
...
big error when being confident. 
