...
Please refer to \secref{sec:kernel_random_features} in the supplementary material for more detail.
...
%\]
%with $A=\Sigma_{w}^{-1(N)}$. 
Updating $\Sigma_{w}$ for all the $D_y$ outputs costs $O( (D_\mathrm{in}D_\mathrm{out})^{2} D_y)$ per one new observation. 
