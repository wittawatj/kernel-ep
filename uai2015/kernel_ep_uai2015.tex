...
%\]
%with $A=\Sigma_{w}^{-1(N)}$. 
Updating $\Sigma_{w}$ costs $O(D_{out}^{2})$ per  new observation. 
For $\mu_{w}= \Sigma_{w} \mathsf{X} \mathsf{Y}^{\top}\sigma_{y}^{-2}$, we maintain
$ \mathsf{X} \mathsf{Y}^{\top}\in\mathbb{R}^{D_{in}\times D_{out}}$ and update it as
\begin{equation}
\left( \feaX \feaY^{\top}\right)^{(N+1)}=\left( \feaX \feaY^{\top}+\mathsf{x}_{N+1}\mathsf{y}^\top_{N+1}\right), 
\end{equation}
costing $O(D_{in}\times D_{out})$ per update. 
