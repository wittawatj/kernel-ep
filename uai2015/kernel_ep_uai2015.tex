...
\maketitle
\begin{abstract}
\wjnote{This is the old abstract. Add online learning ?}We propose
...
\begin{figure}
\centering
\missingfigure{Factor graph for binary logistic regression}
\caption{Factor graph for binary logistic regression}
\label{fig:factor_graph_binlog}
\end{figure}

\paragraph{Logistic} To ensure that the operator is capable of learning the relationship 
of incoming and outgoing messages, we first consider a batch learning setting on the logistic factor 
$f(z|x)=\delta\left(z-\frac{1}{1+\exp(-x)}\right)$ in the context of 
