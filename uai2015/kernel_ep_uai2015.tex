...
\caption{Distributions of incoming messages to logistic factor% $\msg{z_i}{\factor}$
  in four different UCI datasets. 
...
requires us simply to compute the expectation of the sufficient statistic $u(\cdot)$ under the numerator of (\ref{eq:msgPassing:EP}). \wjnote{Under the exact marginal $\msg{ \outV }{\factor}(x_{\outV})\msg{ \factor }{\outV}(x_{\outV})$  ?}
...
In \cite{Rahimi2007}, methods based on Fourier transforms for computing a vector
of random features $\hat{\varphi}$ for the standard Gaussian kernel 
$g(x, y) = \exp\left(-\frac{\| x-y\|^2}{2\beta^2} \right) $ were 
proposed such that $g(x, y) \approx \hat{\varphi}(x)^\top \hat{\varphi}(y)$
where $x,y \in \mathbb{R}^d$ and $\hat{\varphi} \in \mathbb{R}^{D_{in}}$.  
Here we will follow a similar approach and derive 
a set of random Fourier features for \eqref{eq:gauss_joint_emb} via a two-stage 
approximations.

We start by expanding the square in the exponent 
of \eqref{eq:gauss_joint_emb} yielding 
%
\begin{align*}
...
the result of \cite{Rahimi2007} to have 
%
\begin{align*}
 \langle \mu_{\mathsf{r}},\mu_{\mathsf{s}}  \rangle
...
%
by random Fourier features of \citeauthor{Rahimi2007} to obtain a vector 
...
A pseudocode for generating the random features $\hat{\psi}$ is summarized in Algorithm~\ref{algo:random_features_kgg}. For a more detailed derivation, 
please refer to the supplementary materials.
For the random features, we need to pre-compute $\left\{ \omega_{i}\right\} _{i=1}^{D_{in}},\left\{ b_{i}\right\} _{i=1}^{D_{in}},\left\{ \nu_{i}\right\} _{i=1}^{D_{out}}$
...
\begin{algorithm}[t]
\caption{Construction of two-staged random features for $\kappa_{\text{gauss}}$}
...
be the regression outputs where $q^i_{\factor \rightarrow \outV}$ is the numerator 
of \eqref{eq:msgPassing:EP}.
In our application, $\mathsf{x}_{i}$ denotes a random feature
...
\[
\left( \feaX \feaY^{\top}\right)^{(N+1)}=\left( \feaX \feaY^{\top}+x_{N+1}y_{N+1}\right), 
\]
