...
uncertainty estimate $\uncertaintyMsg{\factor}{\outV}{\theta}$. We will regress from the tuple of input messages, which
are probability distributions, to a set of output parameters. To this end we extend a kernel over distributions from \citep{ChristmannSteinwart10} to the case where the input consists of more than one distribution.
...
On the same basis, a mean embedding may be defined on the product of  messages in a tuple,
...
\end{equation}
%
This kernel has two parameters: $\gamma^{2}$, and the width parameter $k$ defining $\mu_{\mathsf{r}} = \mathbb{E}_{\mathsf{r}}k(x,\cdot)$.
