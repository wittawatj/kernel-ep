...
\textbf{Balaji Lakshminarayanan},$^1$ \, \textbf{Dino Sejdinovic}$^2$ \and
\textbf{ Zolt{\'a}n Szab{\'o}}$^1$  \vspace*{2mm} \\
Gatsby Unit, University College London$^1$ \\
\vspace*{2mm}
University of Oxford$^2$ \\
\texttt{ \{wittawatj,  arthur.gretton\}@gmail.com}, \, \texttt{nheess@gmail.com} \\ 
\texttt{ali@arkitus.com}, \, \texttt{balaji@gatsby.ucl.ac.uk},\\
\texttt{dino.sejdinovic@gmail.com},\, \texttt{zoltan.szabo@gatsby.ucl.ac.uk}
}
...
Code to implement our method is available online at \url{https://github.com/wittawatj/kernel-ep}.
...
When the projection  is to a member $q(x|\eta)=h(x)\exp\left(\eta^{\top}u(x)-A(\eta)\right)$ of an exponential family, one simply computes the expectation of the sufficient statistic $u(\cdot)$ under the numerator of (\ref{eq:msgPassing:EP}).
% \wjnote{Under the exact marginal $\msg{ \outV }{\factor}(x_{\outV})\msg{ \factor }{\outV}(x_{\outV})$  ?}
A sample based approximation of this expectation can be obtained via Monte Carlo simulation.   %AG: importance sampling is not MCMC
...
exceeds a pre-defined threshold, the required message is approximated via
importance sampling (cf.\ \eqref{eq:msgIS}) and
$\approxMsg{\factor}{\outV}{\theta}$ is updated
%AG: fixed
...
\end{equation}
%
This kernel has two parameters: $\gamma^{2}$, and the width parameter of the
kernel $k$ defining $\mu_{\mathsf{r}} = \mathbb{E}_{x \sim \mathsf{r}}k(x,\cdot)$.
...
$\kappa(\mathsf{r}, \mathsf{s}) \approx \hat{\psi}(\mathsf{r})^\top \hat{\psi}(\mathsf{s})$
where $\hat{\psi}(\mathsf{r}), \hat{\psi}(\mathsf{s}) \in \mathbb{R}^{D_\mathrm{out}}$. 
...
We refer to \secref{sub:Expected-Product-Kernel} in the supplementary material for more details.
...
embeddings. Both of the kernels depend on a number of parameters. In this section,
...
(with equality in expectation) where $\hat{\phi}(x), \hat{\phi}(y) \in\mathbb{R}^{D}$
...
\item Compute the Fourier transform $\hat{k}$ of the kernel $k$:
    $\hat{k}(\omega)=\frac{1}{2\pi}\int e^{-j\omega^{\top}\delta}k(\delta)\,
    d\delta$.
...
%&=\prod_{i=1}^{c}\kappa^{(i)}\left(p^{(i)},q^{(i)}\right)\\
 &=\prod_{i=1}^{c}k\left(\left(m_{i}-\mu_{i}\right)/w_{i}^{m}\right)
 \prod_{i=1}^{c}k\left(\left(v_{i}-\sigma_{i}^{2}\right)/w_{i}^{v}\right),
\end{align*}
...
which is given by 
\[
c_{x}(t)=\mathbb{E}_{x}\left[\exp\left(itb\right)\right]
=\exp\left(itm-\frac{1}{2}\sigma^{2}t^{2} \right).
\]
For $m=0,t=1$, we have 
\[
c_{b}(1)=\mathbb{E}_{b}\left[\exp(ib)\right]=
\exp\left(-\frac{1}{2}\sigma^{2}\right)=\mathbb{E}_{b}\left[\cos(b)\right],
\]
...
\mathbb{E}_{r(x)}\cos\left(w_{i}^{\top}x+b_{i}\right) & =\mathbb{E}_{r(z_{i})}\cos(z_{i})\\
 ...
 & \overset{(c)}{=}\cos(w_{i}^{\top}m_{r}+b_{i})
 \exp\left(-\frac{1}{2}w_{i}^{\top}V_{r}w_{i}\right),
\end{align*}
...
drawn. For a different type of distribution $r$, we only need to
...
\end{align*}
%
where $\hat{\phi}(R)^{\top}\hat{\phi}(S)=\prod_{l=1}^{c}
\hat{\phi}^{(l)}(r^{(l)})^{\top}\hat{\phi}^{(l)}(s^{(l)})$.
The feature map $\hat{\phi}^{(l)}(r^{(l)})$ can be estimated by applying
the random Fourier features to $k_{\text{gauss }}^{(l)}$and taking
the expectations $\mathbb{E}_{r^{(l)}(a)}\mathbb{E}_{s^{(l)}(b)}$.
...
%=========================================  
\section{MORE DETAILS ON EXPERIMENT 1: BATCH LEARNING }
...
Where random features are used, kernel widths 
...
%repeated hold-out for parameter selection 
...
& \textbf{mean log KL} & \textbf{s.d. of log KL}  \\\hline
 ...
 \textbf{Random features + Gaussian kernel on joint embeddings} (KJIT) & -8.97 & 1.57  \\
