...
\newcommand{\uncert}{v}          %%AG: variable used to denote uncertainty
...
\begin{abstract}
  ...
  \agnote{can we get rid of the massive space below the title?}
...
we regress  directly in the primal from random features of the data \citep{Rahimi2007,Le2013,YanSmoZonWil14}.
...
for additional input/output pairs, where uncertain predictions trigger just-in-time updates
...
a benchmark of our uncertainty estimates, a demonstraction of factor learning
...
and update the regression function, as the incoming messages change.
...
Unlike belief propagation, messages and marginals are projected
onto a member of 
some class of known parametric distributions. 
The message $\msg{ \factor }{i}(x_i)$  from factor $\factor$ to variable $\outV$ is 
%
\begin{equation}
\small
\frac{\projP{ \msg{\outV}{\factor}(x_{\outV})
\int \factor (\fx{\factor}) \prod_{\outV' \in \fis{\factor} \textbackslash \outV} \msg{\outV'}{\factor}(x_{\outV'}) \diffd \bx_{\fis{\factor} \textbackslash \outV}}}
{\msg{\outV}{\factor}(x_{\outV})}
\label{eq:msgPassing:EP}
\end{equation}
%
\aenote{Make equation and number fit on one line}
...
when the projection is to a member of an exponential family, 
$q(x|\eta)=h(x)\exp\left(\eta^{\top}u(x)-A(\eta)\right),$
requires us simply to compute the expectation of $u(\cdot)$ under the numerator of (\ref{eq:msgPassing:EP}). \wjnote{Under the exact marginal $\msg{ \outV }{\factor}(x_{\outV})\msg{ \factor }{\outV}(x_{\outV})$  ?}
...
&\approx \frac{1}{M} \sum_{l=1}^M w(\fx{\factor}^l) u(x_{\outV}^l),~~~~\fx{\factor}^s \sim \tilde{b}
\label{eq:msgIS}
\end{align}
\aenote{Make equation and number fit on one line}
...
\end{equation}
where $\theta$ are the parameters of the function approximator.
...
\end{equation}
where $\uncert$ indicates the expected unreliability  of the predicted, approximate message $\msg{\factor}{\outV}$ returned by $\approxMsg{\factor}{\outV}{\theta}$. If $v = \uncertaintyMsg{\factor}{\outV}{\theta} \left( (\msg{\outV'}{\factor} )_{\outV' \in \fis{\factor}}\right)$ 
...
%=============================================
\section{Kernel learning of message operators}\label{sec:Online}
%=============================================
...
UCI datasets. \aenote{Figure fonts too large.}
...
from a single distribution to a vector output is a straightfoward extension \citep{Szabo2014}, and we
will be able to use kernels from both this work and from \citep{ChristmannSteinwart10}.
...
messages form incoming messages, for the purpose of Belief Propagation. The resulting learning problem
is quite dissimlar to ours, however: the incoming messages in the earlier work are smoothed
conditional density functions, rather than RKHS distribution embeddings; the output messages are likewise
...
we do not express our solution in terms of kernels, but in terms of random features whose expected inner products
are equal to the kernel function. We then perform regression directly in the primal on these random features.
Thus, our first task, in Section \ref{sec:kernelsOnDistributions}, is to define kernels on tupples of distributions, and to
...
Assuming $c$ incoming messages, we write the set of incoming messages to a node as a tuple of
probability distributions $\mathsf{x}_r:=(r^{(l)})_{l=1}^c$, each defined on a respective domain $\mathcal{X}^{(l)}$.
Our goal is to define a kerne between one such tuple, and a second one,
which we will write $\mathsf{x}_s:=(s^{(l)})_{l=1}^c$.

\agnote{Looking at the amount of content: I suspect we will only have space in the main section to present
the kernel we actually use. But in any case, for now, I have included multiple kernels with consistent notation. I  put the definitions of the kernels first, and then left the random features to be defined in the next subsection. Otherwise
there is too much to understand in one equation.}
...
\agnote{here is where you define the rahimi recht basic features, and then derive the two-stage kernel representation. Note
 I did not update anything below, so it contains now some redundant kernel definitions repeated from the last section.}
...
\paragraph{Random features on mean embeddings}
...
\left(-\frac{1}{2}\omega_{i}^{\top}V \omega_{i} \right) \right)_{i=1}^{D_{in}}.
\end{equation*}
...
\agnote{this is all standard so can be made even shorter}
...
be the training regression inputs, and $\mathsf{Y}=\left(\mathbb{E}_{q_{\factor\rightarrow \outV}^{1}}u(x_{\outV})|\cdots|\mathbb{E}_{q_{f\rightarrow \outV}^{N}}u(x_{\outV})\right)\in\mathbb{R}^{D_{y}\times N}$
be the regression outputs.
In our application, $\mathsf{x}_{n}$ will denote a random feature
vector representing $[m_{V\rightarrow f}^{n}]_{V\in\mathcal{V}(f)}$,
as defined in the previous section. Thus, the dimensionality
of  $\mathsf{X}$ is $D_x\times N$, where $D_x$ is the size of the random
feature representation of the entire message tuple.
...
We now review Bayesian linear regression on the random features. We are given the priors
\begin{align*}
...
$\msg{p_i}{\psi} = \text{Beta}(p_i; \alpha, \beta) $. \aenote{Would be nice to have directions on arrows.}
}
...
We use Infer.NET framework \citep{Minka2014} for implementing EP.
...
\begin{figure}[ht]
  ...
  %
  \subfloat[Examples of errors\label{fig:kl_div_4plots}]{
  ...
  factor and examples of predicted messages at different error levels. \aenote{Subfigure heights should match.}}
...
\begin{figure*}[t]
\centering
  \subfloat[Parameters of $\msg{z_i}{\psi}$ \label{fig:logistic_uncertainty_test}]{
  \includegraphics[width=0.47\columnwidth]{img/uncertainty/logistic_uncertainty_test-crop}
  }
  %
  \subfloat[Kernel-based operator \label{fig:logistic_predvar}]{
  \includegraphics[width=0.47\columnwidth]{img/uncertainty/logistic_predvar-crop}
  }
  ...
  \caption{(a) Incoming messages from $z$ to $\psi$ from 20 EP runs in the binary 
  logistic regression as shown in \figref{fig:factor_graph_binlog}.  
  (b) Predictive variances of the proposed method on the two 
  uncertainty test sets shown. For testing, we fix the other incoming message 
  $\msg{p_i}{\psi}$ to $\text{Beta}(p_i; 1, 2)$.
  (c,d) The uncertainty estimates on the same test sets of Breiman's random 
  forests and extremely randomized trees. \aenote{Subfigure heights should match.}}
...
\figref{fig:kl_div}. The forward incoming messages $\msg{z_i}{\psi}$ in the previously 
used training set are shown in \figref{fig:logistic_uncertainty_test}. 
The backward incoming messages $\msg{p_i}{\psi}$ are not shown.
Shown in the same plot are two curves representing two uncertainty test sets
in which each point corresponds to an incoming message. 
The test sets contain a mix of messages in the dense region of 
the training set and messages in unexplored regions.

\figref{fig:logistic_predvar}, \figref{fig:breiman_uncertainty}, and 
\figref{fig:ert_uncertainty} show uncertainty estimates of each point in 
the two test sets by our kernel-based operator, Breiman's random forests 
and extremely randomized trees, where we fix 
$\msg{p_i}{\psi} := \text{Beta}(p_i; 1, 2)$ for testing. 
...
training region, the predictive variance reported by the proposed method increases
much more smoothly compared to that of the random forests. Further, our method
...
in a low density region or unexplored regions. These results ensure that the 
predictive variance is a robust thresholding criterion for querying the oracle.
...
sampler oracle when 
presented with incoming messages very similar to one in the training set, 
thereby unnecessarily wasting computations.
...
\paragraph{Just-in-time learning}
Next we consider a just-in-time learning setting on the same task with toy data. 
...
We sequentially present the operator with 50 related problems where a new 
set of observations $\{(x_i, y_i)\}_{i=1}^n$ are generated at the beginning of 
...
As before, the inference target is the $p(\boldsymbol{w}|\{(x_i, y_i)\}_{i=1}^n)$.
...
in the binary logistic regression problem. \aenote{Mention in caption what the stars and dashed lines represent.}
...
We employ a ``minibatch'' idea in which the operator always consults the oracle in
...
parameters with the median heuristic \citep{Scholkopf2002} 
(see \secref{sec:median_heuristic} in the supplementary materials for details). 
Cross validation can be performed 
quickly by reducing the number of random features and setting it back to the 
desired number once the best parameter combination is chosen. The numbers 
of random features are $D_{in} = 300$ and $D_{out} = 500$. The output noise variance 
$\sigma^2_y$ is fixed to $10^{-4}$ and the uncertainty threshold on the log 
predictive variance is set to -8.5. To simulate a black-box setup, we use 
an importance sampler as the oracle rather than Infer.NET's factor implementation, 
where the proposal distribution is fixed to $\mathcal{z; 0, 200}$ with 50000 particles.

\figref{fig:logistic_temporal_uncertainty} shows a trace of predictive variance 
of KJIT in predicting the mean of each $\msg{\psi}{z_i}$ upon each factor invocation. 
The black dotted lines indicate the start of a new inference problem. 
...
off after the operator sees about 1000 incoming-outgoing message pairs, 
which is relatively low compared to approximately 3000 message passings 
...
observations to collect incoming messages $\msg{z_i}{\psi}$. 
...
$\boldsymbol{w}$ to have zero mean, leading to $\msg{z_i}{\psi}$ also having a 
...
\begin{figure}[t]
  ...
  \caption{Classification performance and inference times of all methods. \aenote{Extra full stop in the title of first subfigure.}
  }
...
sampler for inference (denoted by ``Sampling''), and using the Infer.NET's 
...
is as good as importance sampler and Infer.NET. \figref{fig:logistic_inference_time} 
shows the inference time spent by all methods in each problem. 
While the inference quality is equally good, KJIT is orders of magnitude faster 
than the importance sampler. 
\wjnote{The figure will be updated to reflect this. Experiments running.}
...
\caption{Uncertainty estimate of KJIT for all incoming messages at each time point 
in the compound Gamma problem. \aenote{Top of legend missing. Legend items could be `Predictive variance', `Moving average', `Oracle consultation', `Threshold'. Title should be predictive variance of the \textit{outgoing} message.}
...
% 	}
  ...
  %
  \subfloat[Inferred shape \label{fig:cg_infer_rate}]{
  ...
  while requiring several orders of magnitude less runtime. \aenote{Figure fonts too small.}}
  \label{fig:cg_performance}
\end{figure}


We next explore the compound gamma factor, an alternative to a gamma distribution 
as a heavy-tailed prior distribution on the precision of a normal random variable
...
The task we consider is to infer the posterior of the precision $tau$ of a normally 
...
and let the operator JIT learn to directly send an outgoing message $\msg{\psi}{\tau}$ 
given $\msg{\tau}{\psi}$ using Infer.NET as the oracle. 
The Infer.NET's default implementation in this case relies on a quadrature method.
As in \cite{Eslami2014}, we sequentially present to the operator a number of 
problems where at the beginning of each problem, a random number of observations 
from 10 to 100 and $\tau$ are redrawn from the model.
...
the KJIT with Infer.NET as the oracle. \figref{fig:cg_infer_time} shows the inference 
time of both methods. The plots collectively imply that KJIT can deliver posteriors 
highly agreeing with those obtained from Infer.NET at a much lower cost. 
Note that in this task only one message is passed to the factor in each problem
Thus, \figref{fig:cg_infer_time} indicates that KJIT requires less oracle 
consultations as more problems are seen.

%------------------------------------
\subsection{Real data}
%------------------------------------
standardize the data. Use data in the same sequence as in the plot. 
...
\begin{figure}[ht]
  ...
  \caption{Classification performance and inference times on the four UCI datasets. \aenote{Figure fonts too small. Extraneous full stops in figure titles.}}
...
Let $\psi$ be the logistic factor and $m_{\psi \rightarrow i}$ be an outgoing message. 
Let $q_{\psi \rightarrow i}$ be the ground truth belief message (numerator) associated with 
$m_{\psi\rightarrow i}$. The error metric we use is 
$KL[q_{\psi \rightarrow i}\,||\, \hat{q}_{\psi \rightarrow i}]$
where  $\hat{q}_{\psi \rightarrow i}$ is the belief messages estimated by a learned 
