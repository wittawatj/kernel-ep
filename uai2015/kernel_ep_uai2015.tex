...
\newcommand{\wjnote}[1]{\textbf{\color{blue!80!black}{#1}} }
\newcommand{\aenote}[1]{\textbf{\color{red!50!black}{#1}} }
\newcommand{\nhnote}[1]{\textbf{\color{magenta!80!black}{#1}} }
\newcommand{\agnote}[1]{\textbf{\color{green!70!black}{#1}} }
...
\maketitle
\begin{abstract}
\wjnote{WJ: This is the old abstract. Add online learning ?}We propose
...
refines $m_{f_{i}\rightarrow\theta}$ for each $i$ with \wjnote{WJ: No need to mention this special-case msg.}$m_{f_{i}\rightarrow\theta}(\theta)=\frac{\text{proj}\left[\int dX\, f(X|\theta)m_{X\rightarrow f_{i}}(X)m_{\theta\rightarrow f_{i}}(\theta)\right]}{m_{\theta\rightarrow f_{i}}(\theta)}$
...
\wjnote{WJ: Should jump to this general msg from the beginning.}In general,
...
of incoming messages. \aenote{AE: This sentence needs reworking now that you only train on messages observed in the model.}
...
\aenote{AE: Too many inline equations in this section. Pull out the most important ones?} We consider here the problem of regressing smoothly from distribution-valued
...
of mean parameters.\aenote{AE: Best not to use `in our setting' until section 4 (proposed method). This section is still `background', right?} Although this property is appealing, the training
...
\aenote{AE: Best not to use `we propose' until section 4 (proposed method).}To eliminate the dependency on $N$, we propose to apply random Fourier
...
\eitem Choose the number of particles in the oracle importance sampler so
...
and $m_{Z\rightarrow f}(z)=\text{Beta}(z;\alpha,\beta)$. \aenote{AE: I think at this point you need to properly define the logistic regression model otherwise incoming messages will be incomprehensible.} We randomly
...
to 2000. \aenote{AE: Why 2000?} We report $\log KL[q\|\hat{q}]$ where $q=q_{f\rightarrow X}$
...
instead of the expectations of the first two moments. \aenote{AE: No need to mention log-scaling?} The histogram
...
an appropriate distribution was one of the unsolved problems in \cite{Heess2013}. \aenote{AE: These paragraphs are very informative. Would be great to have them in some form towards the beginning of the paper too. 1. Say what you're going to say. 2. Say it. 3. Say what you just said. there }
