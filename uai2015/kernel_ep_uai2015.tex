...
\end{align*}
where the mappings $\hat{\phi}$ are $D_\mathrm{in}$ standard Rahimi-Recht random features
...
we have
\begin{equation}
\kappa_{\text{gauss}}(\mathsf{r}, \mathsf{s})\approx\exp\left(-\frac{\|\hat{\phi}(\mathsf{r})-\hat{\phi}(\mathsf{r})\|_{D_\mathrm{in}}^{2}}{2\gamma^{2}}\right),
%
...
\begin{algorithmic}[1]
\REQUIRE Input distribution $\mathsf{r}$, Fourier transform $\hat{k}$ of the embedding kernel $k$, number of inner features $D_\mathrm{in}$, number of outer features $D_\mathrm{out}$, outer Gaussian width $\gamma^2$.
...
%\]
%with $A=\Sigma_{w}^{-1(N)}$. 
Updating $\Sigma_{w}$ costs $O(D_\mathrm{out}^{2})$ per one new observation. 
For $\mu_{w}= \Sigma_{w} \mathsf{X} \mathsf{Y}^{\top}\sigma_{y}^{-2}$, we maintain
$ \mathsf{X} \mathsf{Y}^{\top}\in\mathbb{R}^{D_{\mathrm{in}}\times D_{\mathrm{out}}}$ and update it
at cost $O(D_\mathrm{in}\times D_\mathrm{out})$ as
\begin{equation}
...
Below we evaluate our learned message operator using two different factors, the logistic factor, and the compound Gamma factor. In the first and second experiment we demonstrate that proposed operator is generally capable of learning high-quality mappings from incoming to outgoing messages, and that the associated uncertainty estimates are very reliable. The third and fourth experiments assess the performance of the operator as part of the full EP inference loop in two different models, approximating the logistic and the compound gamma factor. Our final experiment highlights the learned operators ability to reliably and quickly adapt to large shifts in the message distribution encountered during inference in a sequence of several real-world regression problems. 


For all experiments we used Infer.NET \citep{Minka2014} with its extensible factor interface for our own operator. 
We use the default settings of Infer.NET unless stated otherwise. In all our experiments the regression target is the marginal belief (numerator of eq.\ \ref{eq:msgPassing:EP}),  from which the outgoing message can be calculated straightforwardly.
% as
...
as described in section \ref{sec:Online}. 
...
where $q_{\factor \rightarrow z_i}$ is the ground truth projected belief (numerator of eq.\ \ref{eq:msgPassing:EP}) and 
...
It is evident that the kernel-based operator captures the relationship
between incoming and outgoing messages well. The discrepancy with respect to the ground truth is barely 
...
To assess this property of we compute the predictive variance using the same learned operator as used in 
...
these are the sets of parameter pairs on which we evaluate
...
follows that of \cite{Eslami2014}. 
