...
We ran EP for 20 trials, where in each trial a new true parameter $\boldsymbol{w}$ was
drawn, along with the observations $\{(x_i, y_i)\}_{i=1}^n$.
The number of EP iterations was set to 10.
We used Infer.NET \citep{Minka2014} for implementing EP, including
its extensible factor interface for our model.
Unless stated otherwise, all configurations followed the default settings of Infer.NET.
In each of the 20 trials, we collected incoming-outgoing message pairs in 
the first five iterations of EP from Infer.NET's default implementation of the 
logistic factor.
All collected messages were randomly partitioned into 5000 training messages and 
...
The number of random features was set to $D_{in}=500$ and $D_{out}=1000$.
Empirically, we observed no significant improvements beyond 1000 random features.
...
{\bf Compound gamma factor:} We next simulate the compound gamma factor, % an alternative to a gamma distribution as
...
{\bf Classification benchmarks:} In the final experiment, we demonstrate
...
and to query these estimates 

the high flexibility of the operator 
