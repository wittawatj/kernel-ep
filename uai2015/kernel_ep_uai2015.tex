...
The message $\msg{ \factor }{i}(x_i)$  from factor $\psi$ to variable $i$ is 
%
\begin{equation}
\small
...
\end{equation}
%
where $\msg{i'}{\factor}$ are the messages sent to factor $\psi$ from its neighboring variables $x_{i'}$ other than $x_i$,  $\projP{p} = \argmin{q \in \mathcal{Q}} \kld{p}{q}$, and $\mathcal{Q}$ is typically in the exponential family, e.g.\ the set of Gaussian or Beta distributions.
...
requires us simply to compute the expectation of $u(\cdot)$ under the numerator of (\ref{eq:msgPassing:EP}). \wjnote{Under the exact marginal $\msg{ i }{\factor}(x_i)\msg{ \factor }{i}(x_i)$  ?}
...
&\approx \frac{1}{M} \sum_{l=1}^M w(\fx{\factor}^l) u(x_i^l),~~~~\fx{\factor}^s \sim \tilde{b}
\label{eq:msgIS}
\end{align}
...
exceeds a pre-defined threshold, the required message is approximated via importance sampling (cf.\ eq.\ \ref{eq:msgIS}) and $\approxMsg{\factor}{i}{\theta}$ is updated
%AG: fixed
