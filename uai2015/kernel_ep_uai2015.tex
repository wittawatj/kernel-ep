...
conjugate prior for the standard deviation, has quite pathological behaviour \citep{Gelman2006}. \todo{AE: suggestion: `the inverted Gamma prior, which is widely used due to being a convenient conjugate prior' or something similar}
...
for balanced trees.
Finally, note that the approach of Ali et al. uses certain bespoke features of the factors when
...
{\bf Computational Complexity of random forest approach:}
...
at the leaves, $N$ be the number of points provided by the importance sampling oracle, and $M$ be the
...
be re-trained from scratch for each new point. Retraining at a leaf costs $O(D_r^2 (D_r + M))$, however $M$ is  typically negligible at a lower depth, therefore cost is $O(D_r^3)$.
Therefore the total updating cost across all trees is $O(K D_r^3 D_t log(N))$.

It is instructive to consider some representative numbers used by Ali et al. 
...
and the number of samples   $M$ per leaf was between 10 and 50.
