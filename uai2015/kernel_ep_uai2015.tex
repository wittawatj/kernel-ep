...
\begin{abstract}
  ...
  \wjnote{Tried vspace -3cm but did not work. The space is for author list ?}
...
\caption{Incoming messages $\msg{z_i}{\factor}$ in the online learning on the four
UCI datasets. 
...
the outgoing message $\msg{\factor}{z_i}$ to sidestep the issue. 
...
\aenote{Subfigure heights should match.}}
...
$\hat{q}_{\factor \rightarrow z_i}$ is the predicted.
...
\centering
  ...
  \includegraphics[width=0.49\columnwidth]{img/uncertainty/logistic_predvar-crop}
  }
  ...
  \caption{(a) Incoming messages from $z$ to $\factor$ from 20 EP runs in the binary 
  ...
  \wjnote{Incorporate Balaji's latest figure. There is only one ?}
  }
...
The backward incoming messages $\msg{p_i}{\factor}$ are not shown.
...
We sequentially present the operator with 30 related problems where a new 
...
\caption{Uncertainty estimate of KJIT of outgoing messages at each factor invocation
in the binary logistic regression problem. The black dashed lines indicate the start 
...
where the proposal distribution is fixed to $\mathcal{N}(z; 0, 200)$ with 
...
\caption{
Uncertainty estimate of KJIT of outgoing messages on the four UCI datasets.
...
and let the operator JIT learn to directly send an outgoing message $\msg{\factor}{\tau}$ 
...
In the final experiment, we demonstrate the high flexibility of the operator 
under a change of the distribution of incoming messages. We consider four real 
...
before. As a stress test, we let the operator JIT learn to send outgoing messages 
$\msg{\factor}{z_i}$ and $\msg{\factor}{p_i}$ on the four problems presented 
sequentially. The training observations consist of 200 data points subsampled
from each dataset by stratified sampling. 
For the fertility dataset which contains only 
100 data points, we subsample half of them. The rest of the data is used as 
classification test sets. The uncertainty threshold is set to -9 and the minibatch 
size is 500. All other parameters are the same as in the JIT learning experiment in
\secref{sec:logistic_factor}.
...
The results positively suggest that KJIT improves the inference time on all the 
problems without sacrificing inference accuracy. The trace of the predictive 
...
that the operator is able to promptly detect a change of input distribution
and robustly adapt to the new input distributions by querying the oracle.
