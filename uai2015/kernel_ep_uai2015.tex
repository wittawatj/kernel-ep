...
We now propose a kernel method for learning the message operator $\approxMsg{\factor}{i}{\theta}$ and
uncertainty estimator $\uncertaintyMsg{\factor}{i}{\theta}$
...
probability distributions $(r^{(l)})_{l=1}^c$, each defined on a respective domain $\mathcal{X}^(l)$.
Our goal is to define a kerne between one such tuple, and a second one,
which we will write ADD


$X=\left(x_{1}|\cdots|x_{N}\right)\in\mathbb{R}^{D\times N}$,
where $x_{1}$
where $N$ is the number of obseved training messages, and 
...
vector for $[m_{V\rightarrow f}^{n}]_{V\in\mathcal{V}(f)}$.
...
%------------------------------------
\subsection{Ridge Regression for operator prediction}\label{sec:ridgeRegression}
%------------------------------------
