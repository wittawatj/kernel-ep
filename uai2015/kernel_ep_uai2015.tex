...
\todo[inline]{The following content needs to be rewritten and is not coherent. I just copied it from my notes. Some will be moved to the main text.  }
...
whose feature map $\phi$ cannot be explicitly computed. In \cite{Le2013,Rahimi2007},
...
approximation with higher computational cost. 
...
Fourier features for a translation invariant kernel. Assume $k(x,y)=k(x-y)$
...
\paragraph{Random Features for Expected Product Kernel}

Given two distributions (messages) $p$ and $q$, the expected product
...
is translation invariant i.e., $k(x,y)=k(x-y)$. Assume the two incoming
messages are Gaussian i.e.g, $p(x)=\mathcal{N}(x;m_{p},V_{p})$ and
$q(y)=\mathcal{N}(y;m_{q},V_{q})$ ($d$-dimensional Gaussian), By
applying the previous result for random Fourier features on $k$,
it can be shown (detail omitted) that the random features for $\kappa(p,q)\approx\hat{\phi}(p)^{\top}\hat{\phi}(q)$
are given by
...
\end{array}\right)
\]
where $\{w_{i}\}_{i=1}^{D}\sim\hat{k}$ (the Fourier transform of
$k$) and $\{b_{i}\}_{i=1}^{D}\sim U[0,2\pi]$. 
...
\paragraph{Kernel on tuples of messages}
