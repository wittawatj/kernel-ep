...
width=27mm] at (4, -0.5) {set of $c$ variables connected to $f$};
\node (proj_msg_def) [right, fill=green!15, ellipse, inner sep=1pt,
align=center, text width=15mm] at (9, -0.5) {projected message};
\end{tikzpicture}
...
\begin{align*}
  ...
  \frac{	
  \tikz[baseline]{
	  \node[fill=green!15,anchor=base] (projected)
...
text width=70mm]  at (1.8, 1.2) {$\proj[r_{f \rightarrow
...
width=17mm]  at (9.2, 1.2) {incoming message from $V_j$}; 
\end{tikzpicture}
