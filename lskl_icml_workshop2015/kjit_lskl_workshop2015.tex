...
\icmladdress{$^*$Gatsby Unit, University College London, London, UK}
\vspace*{-3mm}
...
or undermine intended models. %\wjnote{Is it SD or variance ?}
% AG: variance - fixed :)
...
In this work, we address the particular setting of expectation propagation \citep{Minka2001}, a message
...
problem, where the inputs are probability measures, and the outputs are vectors of message
...
for additional input/output pairs. 
...
factors implemented as forward sampling functions, fully
...
One approach, due to \citet{Barthelme2011}, is to compute the message update
...
but this approach can suffer from high computational cost when a large number of 
particles is used to get accurate estimates.

To obtain low-variance message approximations at lower computational cost
\cite{Heess2013} and \cite{Eslami2014} both amortize previously computed
approximate messages by training a function approximator to directly map a
tuple of incoming variable-to-factor messages $(\msg{\outV'}{\factor} )_{\outV'
    \in \fis{\factor}}$ to an approximate factor to variable message
...
exceeds a pre-defined threshold, the required message is approximated via importance sampling  and $\approxMsg{\factor}{\outV}{\theta}$ is updated
%AG: fixed
...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.46\columnwidth]{kl_ntr5000_iter5_sf1_st20_fm_joint-crop}
  }
  %
  \subfloat[Examples of predictions\label{fig:kl_div_4plots}]{
  \includegraphics[width=0.47\columnwidth]{4plots_ntr5000_iter5_sf1_st20_fm_joint-crop}
  }
...
\centering
  ...
  \includegraphics[width=0.49\columnwidth]{uncertainty/logistic_uncertainty_test-crop}
  }
  %
...
%   }
  ...
  \includegraphics[width=0.49 \columnwidth]{uncertainty/logistic_uncertainty_all-crop}
  }
  %
...
\begin{figure}[t]
  ...
  \includegraphics[width=0.49\columnwidth]{online/logistic_01_loss-crop}
  }
  %
  \subfloat[Inference time\label{fig:logistic_inference_time}]{
  \includegraphics[width=0.47\columnwidth]{online/logistic_inference_time-crop}
  }
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{online/uci_in_msgs-crop}
