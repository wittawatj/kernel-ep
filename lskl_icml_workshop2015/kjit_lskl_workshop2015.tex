...
In this work, we address the particular setting of expectation propagation (EP)
\citep{Minka2001}, a message passing algorithm wherein messages are confined to
being members of a particular parametric family. Sending EP outgoing messages
requires a difficult computation involving integrating incoming messages over a
...
$\approxMsg{\factor}{\outV}{\theta}$ is updated on this new datapoint (leading
...
(messages) \citep[eq.  9]{Christmann2010}.
...
Below we evaluate our learned message operator using the logistic factor.  For
all experiments we used Infer.NET \citep{Minka2014} with its extensible factor
interface for our own operator.  We use the default settings of Infer.NET
