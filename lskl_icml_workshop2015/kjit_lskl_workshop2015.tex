...
\icmlauthor{Wittawat Jitkrittum$^1$}{wittawatj@gmail.com}
\icmlauthor{Arthur Gretton$^1$}{arthur.gretton@gmail.com}
\icmlauthor{Nicolas Heess$^*$}{nheess@gmail.com}
\icmlauthor{S. M. Ali Eslami$^*$}{ali@arkitus.com}
\icmlauthor{Balaji Lakshminarayanan$^1$}{balaji@gatsby.ucl.ac.uk}
\icmlauthor{Dino Sejdinovic$^2$}{dino.sejdinovic@gmail.com}
\icmlauthor{Zolt{\'a}n Szab{\'o}$^1$}{zoltan.szabo@gatsby.ucl.ac.uk}

\icmladdress{$^1$Gatsby Unit, University College London}
\vspace*{-3mm}
\icmladdress{$^2$University of Oxford}
...
$\mu_{r^{(l)}}(\cdot) := \int k^{(l)}(x^{(l)},\cdot) \, \mathrm{d}r^{(l)} (x^{(l)})$.
%
...
\int k([x^{(1)}, \ldots, x^{(c)}],\cdot) \, \mathrm{d}\mathsf{r}(x^{(1)}, \ldots, x^{(c)}),$
...
Instead, we define 
a finite-dimensional random feature map $\hat{\psi}(\mathsf{r}), \hat{\psi}(\mathsf{s}) \in \mathbb{R}^{D_\mathrm{out}}$ such that 
...
\end{align*}
%
where the mappings $\hat{\phi}(\mathsf{r}), \hat{\phi}(\mathsf{s})$ are $D_\mathrm{in}$ standard Rahimi-Recht
...
of random features $\hat{\psi}(\mathsf{r}), \hat{\psi}(\mathsf{s}) \in \mathbb{R}^{D_\mathrm{out}}$ such that 
