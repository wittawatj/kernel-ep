...
namespace MicrosoftResearch.Infer.Learners
{
    ...
    public class StarRatingRecommenderEvaluator<TInstanceSource, TUser, TItem, TGroundTruthRating>
        : RecommenderEvaluator<TInstanceSource, TUser, TItem, TGroundTruthRating, int, Discrete>
    {
        ...
        private static double RatingMetricExpectation(
             int trueRating, Discrete predictedRatingDistribution, Func<int, int, double> metric)
        {
            double result = 0;
            for (int predictedRating = 0; predictedRating < predictedRatingDistribution.Dimension; ++predictedRating)
            {
                result += metric(trueRating, predictedRating) * predictedRatingDistribution[predictedRating];
            }
