...
namespace MicrosoftResearch.Infer.Learners
{
        ...
        IRecommender<TInstanceSource, TUser, TItem, int, Discrete, TFeatureSource>
    {
        ...
        public Discrete PredictDistribution(TUser user, TItem item, TFeatureSource featureSource = default(TFeatureSource))
        {
            ...
            Vector probs = dirichlet.Sample();
            double[] shiftedProbs = Util.ArrayInit(
                this.starRatingInfo.MaxStarRating + 1,
                i => i < this.starRatingInfo.MinStarRating ? 0 : probs[i - this.starRatingInfo.MinStarRating]);
            return new Discrete(shiftedProbs);
        }
