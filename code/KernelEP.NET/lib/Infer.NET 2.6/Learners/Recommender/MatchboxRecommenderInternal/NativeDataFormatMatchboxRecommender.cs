...
namespace MicrosoftResearch.Infer.Learners.MatchboxRecommenderInternal
{
    ...
    internal class NativeDataFormatMatchboxRecommender<TInstanceSource, TFeatureSource> :
        IMatchboxRecommender<TInstanceSource, int, int, TFeatureSource>
    {
        ...
        private InstanceMetadata GetInstanceMetadata(TInstanceSource instanceSource, TFeatureSource featureSource)
        {
            ...
            var userFeatures = this.Settings.Training.UseUserFeatures
                ? SparseFeatureMatrix.Create(nonZeroUserFeatureValues, nonZeroUserFeatureIndices)
                : SparseFeatureMatrix.CreateAllZero(userCount);
            var itemFeatures = this.Settings.Training.UseItemFeatures
                ? SparseFeatureMatrix.Create(nonZeroItemFeatureValues, nonZeroItemFeatureIndices)
            ...
            try
            {
                Parallel.For(
                    ...
                    batchNumber =>
                    {
                        ...
                        this.SetupCommunityTrainingAlgorithm(currentBatchAlgorithm, instanceData, metadata, constraints, this.parameterPosteriorDistributions);
