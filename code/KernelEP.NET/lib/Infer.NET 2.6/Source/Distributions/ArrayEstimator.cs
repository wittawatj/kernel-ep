...
namespace MicrosoftResearch.Infer.Distributions
{
    ...
    public class ArrayEstimator
    {
            ...
            where T : IDistribution<TDomain>
        {
            ...
            Array distributions = (Array) method.Invoke(null, new object[] {dist});
            ...
            Array estimators = JaggedArray.ConvertToNew(
                distributions, leafDistributionType, leafEstimatorType, elt => method.Invoke(EstimatorFactory.Instance, new object[] {elt}));

            int depth = domainTypes.Length;
            if (depth > 4)
                throw new ApplicationException("Estimator arrays of depth greater than 3 not supported");
            ...
            if (arrType.IsArray)
            {
                ...
                foreach (MethodInfo sm in staticMeths)
                {
                    if (sm.Name == "Array")
                    {
                        ...
                        {
                            result = mi.Invoke(null, new object[] {arrayOfEstimators, estimatorTypes});
                            break;
                        }
                    }
                }
    ...
    public static class ArrayEstimator<T>
    {
            ...
            where TEstimator : Estimator<T>
        {
            ...
            return (TEstArray) method.Invoke(null, new object[] {array, estArrTypes[0]});
        }
